...
def encrypt(file_name,key_file):
    ...
    not_encrypted_file_name = base64.b64encode(file_name.split('/')[-1].split('\\')[-1].encode()).decode()
    encrypted_file_name = ''
    ...
    with open(encrypted_file_name,'w') as encrypted_file:
        not_encrypted_file = base64.b64encode(open(file_name, "rb").read()).decode("utf-8")
        for symbol in key:
            required_codes[symbol] = 0
